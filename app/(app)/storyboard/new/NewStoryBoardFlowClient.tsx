...
const QUESTIONS = [
  ...
  },
  {
    ...
    sample:
      "I mapped the journey into five steps, tagged drop-off in our analytics tool, interviewed twelve new users, and modeled where time-to-value spiked. That showed most exits clustered around step three and the paywall, so we knew where to dig first.",
  },
  {
    id: 3,
...
export default function NewStoryBoardFlowClient() {
  ...
  useEffect(() => {
    if (!experienceId) router.replace("/storyboard");
  }, [experienceId, router]);
  ...
  const beginSpeechHold = () => {
    ...
    recognition.onresult = (event: WebSpeechResultEvent) => {
      ...
      const combined =
        !sessionTrim ? base : !base ? sessionTrim : `${base} ${sessionTrim}`;
  ...
  const handleNext = () => {
    if (isLast) {
      ...
      router.push(`/storyboard/crafting?experienceId=${encodeURIComponent(experienceId)}`);
      return;
    }
  ...
  return (
    <div className={`${urbanist.className} relative min-h-[calc(100vh-64px)] overflow-x-hidden pb-44`}>
      <div className="relative z-[2] mx-auto w-full max-w-[1440px] px-6 py-6">
        <div className="mx-auto mb-8 flex w-full max-w-[684px] justify-start">
          <button
            type="button"
            onClick={handlePrev}
        ...
        <section className="relative z-[1] flex w-full flex-col items-center gap-8 px-4 sm:px-8">
          <div className="w-full max-w-[684px]" data-journey-id="story-progress">
          ...
          <div className="w-full max-w-[684px] text-left" data-journey-id="story-question">
          ...
          <div className="w-full max-w-[684px]" data-journey-id="story-protip">
          ...
          <div className="flex w-full max-w-[684px] flex-wrap items-center justify-between gap-4 border-t border-[#E2E8F0]/90 pt-8">
            <button type="button" onClick={handlePrev} className="text-[13px] font-medium text-[#64748B] hover:text-[#1E293B]">
              Previous
            </button>
            <button
              type="button"
